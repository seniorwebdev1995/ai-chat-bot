...
export default function Notification() {
  ...
  const handelAccept = (creator_id, pid) => {
    instance
      .post('/post/acceptInvite', { creator_id, pid })
      .then((res) => {
  ...
  const handelDecline = (creator_id, pid) => {
    instance
      .post('/post/declineInvite', { creator_id, pid })
      .then((res) => {
        console.log('get declineInvite-----', res.data);
        getNotifications();
      })
      .catch((err) => {
        if (err.response?.status === 401) {
  ...
  return (
    <div className="w-full h-screen px-9 pt-8">
      ...
      {notifications.length > 0 ? (
                ...
                <div key={i} className="flex justify-around items-start">
                  ...
                  <div className="">
                    ...
                    <div className="flex mt-2">
                      <IconButton
                        onClick={() => {
                          handelAccept(item.from.id, item.id);
                        }}
                      ...
                      <IconButton
                        onClick={() => {
                          handelDecline(item.from.id, item.id);
                        }}
        ...
        <div className=" flex justify-center items-center flex flex-col mx-12">
          <SvgIcon
