...
export default function Comments() {
  ...
  const sendMessage = () => {
    instance
      ...
      .then((res) => {
        ...
        setTempComments([res.data, ...temp_comments]);
        setMsg('');
      })
      .catch((err) => {
        if (err.response.status === 401) {
  ...
  const goBack = () => {
    if (state.info) {
      navigate('/news', { state: { info: state.info } });
    } else {
      navigate('/news');
    }
  };
              ...
              {comments.map((item, i) => {
                ...
                if (uid === item_userID || !item_userID) {
                  return (
                    ...
                    >
                      <p className=" text-bg-white break-words">
                        {item.content}
                      </p>
                  ...
                  return (
                    <div key={i} className="inline-flex w-full mt-5">
                      ...
                      />
                      <div className="flex flex-col p-3 bg-bg-gray w-4/5 ml-3 rounded-xl mr-8">
                        <p className="my-2 text-bg-primary text-ssm">
                          {item.userId.user_name}
                        </p>
                        <p className="text-base break-words">{item.content}</p>
