...
export default function News() {
  ...
  const [pageType, setPageType] = useState(state?.info ? 1 : 0);
  ...
  const handleClose = () => {
    // setOpen(false);
    // alert('FC invitation has successfully sent !');
    // navigate('/news/fcdetail');
  };
  ...
  const handleComment = (id) => {
    if (pageType === 0) {
      ...
      navigate(`/news/comments`, {
        state: { id, info: [crewText, creator, creatorName] },
      });
    }
  };
  ...
  const texts = ['Hangar', 'Flight Crews'];

  const getFCMessages = (creator_id) => {
    instance
  ...
  const goFlightCrew = (creator_id, text, creator_name) => {
    setCrewText(text);
    setCreator(creator_id);
    setCreatorName(creator_name);
    setCurrentIndex(0);
    getFCMessages(creator_id);
      ...
      <div className="inline-flex w-full justify-center bg-bg-primary">
        {pageType === 0 ? (
              ...
              <div className="flex justify-center">
                {currentIndex === 0 ? (
                  <>
                    ...
                    <div
                      className="rounded-full w-2 h-2 bg-bg-white opacity-50 ml-1"
                      onClick={(e) => switchNF(texts[0])}
                    ></div>
                  </>
                ) : (
                  <>
                    <div
                      className="rounded-full w-2 h-2 bg-bg-white opacity-50 mr-1"
                      onClick={(e) => switchNF(texts[1])}
                    ></div>
          ...
          <div className="inline-flex w-full justify-between pt-8 pb-10 px-9 bg-bg-primary items-center">
            ...
            <p className="text-bg-white text-ti font-semibold !font-futura text-center w-full text-ellipsis overflow-hidden whitespace-nowrap ">
              {crewText}
            </p>
            <div className="stroke-bg-white">
              <button onClick={handleOpen}>
                {/* <SvgIcon
                  ...
                  sx={{ fontSize: '32px' }}
                ></SvgIcon> */}
                <MoreIcon sx={{ color: 'white', fontSize: '32px' }} />
              </button>
              <Modal
                open={modalOpen}
                onClose={handleClose}
                ...
                return (
                  ...
                  >
                    <Avatar
                      ...
                      className="my-4"
                    />
                    <p className="text-center text-text-gray text-ellipsis overflow-hidden w-3/4 whitespace-nowrap ">
                      {item.creator.user_name}
                    </p>
                    <p className="text-center text-ellipsis overflow-hidden w-3/4 whitespace-nowrap">
                      {item.fc_name}
                    </p>
                  </button>
                );
          ...
          {messages.map((item, i) => {
            ...
            if (uid === item_userID || !item_userID) {
              return (
                ...
                >
                  <p className=" text-bg-white break-words">{item.content}</p>
