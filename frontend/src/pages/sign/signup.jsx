...
export default function SignUp() {
  ...
  const [pc_number, setPC] = useState('');
  ...
  const handleContinue = () => {
    if (activeStep === 0) {
      ...
      if (!loading) setActiveStep((prevActiveStep) => prevActiveStep + 1);
      ...
      await instance
        ...
        .then((res) => {
          ...
          if (result.length > 0) {
            ...
            // }
            result.map((item, idx) => {
              if (item.startsWith('PC')) setPC(item);
              if (item.startsWith('C-')) setRPAS(item);
            });
            console.log('result --------', result);
            if (result[1] !== '' && result[1].startsWith('PC')) {
              setPC(result[1]);
            }
            if (result[2] !== '' && result[2].startsWith('C')) {
              setRPAS(result[2]);
            }
  ...
  return (
    <div className="w-full h-screen px-9 pt-8">
      ...
      {activeStep === 0 ? (
        ...
        <>
          <p className="text-ti font-semibold flex justify-center content-center !font-futura my-8 text-center">
            Your Pilot information
          </p>
          <div className="input-field mb-4">
            <p className="text-text-gray my-2">Pilot certification number</p>
            <InputBase
              value={pc_number}
              // onChange={handleChange}
          ...
          <div className="input-field mb-4">
            <p className="text-text-gray my-2">RPAS registration number</p>
            <InputBase
              value={rpas_number}
              // onChange={handleChange}
              placeholder="Please upload file"
      ...
      <div className="flex text-center justify-center mt-5 left-0 right-0 ml-auto mr-auto">
